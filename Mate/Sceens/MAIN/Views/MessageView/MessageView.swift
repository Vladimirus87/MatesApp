...
//  JobnetAPP
//
//  Created by Vladimirus on 02.07.2021.
...
class MessageView: UIView {
    ...
    func showOnView(message:String,theme:Theme){
        ...
        self.frame = CGRect(x: parentView.frame.minX, y: parentView.frame.minY - frame.height, width: frame.width, height: frame.height)
    ...
    func showOnViewWithError(_ error: String) {
        self.showOnView(message: error, theme: .error)
    }
    
    func showError(_ error: Error) {
        self.showOnView(message: error.localizedDescription, theme: .error)
    }
